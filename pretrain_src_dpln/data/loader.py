...
def build_dataloader(task, dataset, collate_fn, is_train: bool, opts):
    ...
    batch_size = opts.train_batch_size if is_train else opts.val_batch_size #训练时用训练batch_size，验证时用验证batch_size
    ...
    if opts.local_rank == -1:
        if is_train:
            ...
            ] = RandomSampler(dataset) #自动选择采样器：训练时用RandomSampler（打乱数据），验证时用SequentialSampler；分布式训练时用DistributedSampler。
        else:
    ...
    return loader, pre_epoch #通过pre_epoch回调函数设置分布式采样器的epoch
