...
def main(opts):
    ...
    local_paths = [
        ...
        "./pretrained_models/xlm-roberta-base",
    ...
    for step, (name, batch) in enumerate(meta_loader):
        ...
        with accelerator.accumulate(model):
            ...
            if accelerator.sync_gradients:
                ...
                # 同步全局步数到模型（保留强化学习相关逻辑）
                unwrapped_model = accelerator.unwrap_model(model)
                if hasattr(unwrapped_model, 'update_global_training_step'):
                    unwrapped_model.update_global_training_step(global_step)
                    ...
                    if accelerator.is_main_process:
                        ...
                        if hasattr(unwrapped_model, 'lambda_coef'):
                            accelerator.log({
                                ...
                                'dual_policy/weight': unwrapped_model.dual_policy_weight.item() if hasattr(
                                    unwrapped_model, 'dual_policy_weight') else 0
                            }, step=global_step)
                    gc.collect()
                    accelerator.print('===============================================')
                ...
                if global_step % opts.valid_steps == 0:
                    ...
                    validate(model, val_loaders_prepared, accelerator, setname='_seen', train_meta_loader=meta_loader)
                    accelerator.print(f'------Step {global_step}: start validation unseen------')
                    validate(model, val2_loaders_prepared, accelerator, setname='_unseen', train_meta_loader=meta_loader)
                    gc.collect()
                    # 只在主进程保存模型
                    if accelerator.is_main_process:
                        # 获取原始模型（去除DDP包装）
...
def validate(model, val_dataloaders, accelerator, setname='', train_meta_loader=None):
    model.eval()
    ...
    # 新增：清理训练集缓存
    if train_meta_loader is not None:
        for name, loader in train_meta_loader.name2loader.items():
            if hasattr(loader.dataset, 'nav_db'):
                if hasattr(loader.dataset.nav_db, '_feature_store'):
                    loader.dataset.nav_db._feature_store.clear()
                if hasattr(loader.dataset.nav_db, '_feature_store_aug'):
