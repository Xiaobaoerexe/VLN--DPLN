...
class GlocalTextPathNavCMT(BertPreTrainedModel):
    ...
    def forward_navigation_per_step(
        ...
        global_adjustment=None, local_adjustment=None,
    ):
        if global_adjustment is None:
            global_adjustment = torch.zeros(1).to(txt_embeds.device)
        if local_adjustment is None:
            local_adjustment = torch.zeros(1).to(txt_embeds.device)
        ...
        global_logits = self.global_sap_head(gmap_embeds).squeeze(2) * fuse_weights + global_adjustment
        global_logits.masked_fill_(gmap_visited_masks, -float('inf'))
        global_logits.masked_fill_(gmap_masks.logical_not(), -float('inf'))

        local_logits = self.local_sap_head(vp_embeds).squeeze(2) * (1 - fuse_weights) + local_adjustment
    ...
    def forward(self, mode, batch, dual_policy_outs = None, **kwargs):
        if mode == 'language':
            ...
            if dual_policy_outs is not None:
                return self.forward_navigation_per_step(
                    ...
                    dual_policy_outs['global_adjustment'], dual_policy_outs['local_adjustment'],
                )
            else:
                return self.forward_navigation_per_step(
                    batch['txt_embeds'], batch['txt_masks'], batch['gmap_img_embeds'],
