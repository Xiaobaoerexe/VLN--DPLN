...
def parse_args():
    ...
    parser.add_argument("--features", type=str, default='clip-h14')
    parser.add_argument('--obj_features', type=str, default='timm_vitb16')
...
def postprocess_args(args):
    ...
    # Setup input paths
    ft_file_map = {
        'clip-b16': 'clip_vit-b16_mp3d_original.hdf5',
        'clip-h14': ['clip_vit-h14_mp3d_hm3d_gibson.hdf5', 'clip_vit-h14_mp3d_original.hdf5'],
        'vitbase-l14': 'pth_vit_base_patch16_224_imagenet.hdf5',
    }
    # 处理img_ft_file - 支持单文件或列表
    ft_file_config = ft_file_map[args.features]
    if isinstance(ft_file_config, list):
        args.img_ft_file = [os.path.join(ROOTDIR, 'R2R', 'features', f) for f in ft_file_config]
    else:
        args.img_ft_file = os.path.join(ROOTDIR, 'R2R', 'features', ft_file_config)
    if args.features == 'vitbase-l14':
        args.val_ft_file = os.path.join(ROOTDIR, 'R2R', 'features', 'pth_vit_base_patch16_224_imagenet.hdf5')
