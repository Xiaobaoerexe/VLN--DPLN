...
class Seq2SeqAgent(BaseAgent):
    ...
    def __init__(self, args, env, rank=0, accelerator=None):
        ...
        self.critic_optimizer = optimizer(self.critic.parameters(), lr=self.args.lr * 0.8, weight_decay=1e-4,
                                          amsgrad=True)
        # 双策略优化器（如果启用）
        ...
        if self.use_dual_policy and hasattr(self.vln_bert, 'dual_policy_finetune'):
            ...
            self.reward_optimizer = optimizer(reward_params, lr=self.args.lr * self.args.reward_actor_lr,
                                              weight_decay=1e-5)

            # Penalty Actor优化器（学习率略低）
            penalty_params = [p for n, p in self.vln_bert.named_parameters() if 'penalty_actor' in n]
            self.penalty_optimizer = optimizer(penalty_params, lr=self.args.lr * self.args.penalty_actor_lr,
                                               weight_decay=5e-5)
