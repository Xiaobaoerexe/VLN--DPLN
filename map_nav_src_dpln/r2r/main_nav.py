...
from utils.logger import write_to_record_file, timeSince
...
def log_training_stats(listner, writer, idx, record_file):
    ...
    write_to_record_file(
        ...
        f"IL_loss {IL_loss:.4f}, critic_loss {critic_loss:.4f},"
        f"r_a_loss {reward_actor_loss:.4f}, p_a_loss {penalty_actor_loss:.4f},"
        f"avg_r {avg_reward:.4f}, avg_p {avg_penalty:.4f}", record_file
    )
    # 返回统计信息字典供tqdm使用
    stats = {
        'IL_loss': IL_loss,
...
def evaluate(listner, val_envs, accelerator, record_file):
    ...
    loss_str = "validation"
    # 创建评估进度条
    eval_pbar = tqdm(val_envs.items(), desc="Evaluating", leave=False)
...
def build_dataset(args, rank=0, is_test=False):
    ...
    val_env_names = ['val_train_seen', 'val_unseen']
    ...
    for split in val_env_names:
        ...
        if args.dataset == 'rxr' and (not is_test) and args.downsample:
            val_instr_data = val_instr_data[::8]
        if args.aug is not None:
            val_feat_db = ImageFeaturesDB(args.val_ft_file, args.image_feat_size)
            val_env = dataset_class(
                val_feat_db, val_instr_data, args.connectivity_dir,
                batch_size=args.batch_size,
                angle_feat_size=args.angle_feat_size, seed=args.seed + rank,
            ...
            val_env = dataset_class(
                feat_db, val_instr_data, args.connectivity_dir,
                batch_size=args.batch_size,
                angle_feat_size=args.angle_feat_size, seed=args.seed + rank,
                sel_data_idxs=None, name=split,
            )
...
def train(args, train_env, val_envs, aug_env=None):
    ...
    for idx in pbar:
        ...
        if aug_env is None:
            ...
            jdx_length = interval // (args.aug_times+1)
            if accelerator.is_main_process:
...
def valid(args, train_env, val_envs):
    ...
    for env_name, env in val_pbar:
        ...
        output_file = os.path.join(args.pred_dir, f"{prefix}_{env_name}.json")
