...
def build_test_dataset(args, rank=0):
    ...
    for split in val_env_names:
        ...
        if args.dataset == 'rxr' and args.downsample:
...
def test_single_model(model_path, args, train_env, val_envs, accelerator):
    ...
    try:
        ...
        if is_pretrained:
            ...
            # 创建agent（会自动加载预训练权重）
            agent = GMapNavAgent(args, train_env, accelerator=accelerator)
            epoch = 0
        else:
...
def main():
    ...
    if accelerator.is_main_process:
        ...
        # 找出当前GPU的最佳模型
        print(f"\n{'=' * 80}")
        print(f"GPU {gpu_id}: BEST MODEL ANALYSIS (among {len(gpu_model_files)} models)")
        print(f"{'=' * 80}")
