...
obj_ft_dim=768
seed=4
MODELS_DIR=${DATA_ROOT}/R4R/exprs_map/pretrain/cmt-vitbase-mlm.mrc.sap-init.lxmert-aug.speaker-RAM-l14/ckpts
# 输出目录
outdir=${DATA_ROOT}/R4R/exprs_map/batch_test_obj
...
if successful_results:
    ...
    best_model = max(successful_results,
                    key=lambda x: x['results']['val_unseen_sampled']['score_summary']['sr']
                    if 'val_unseen_sampled' in x['results'] else 0)

    print(f"\nBest model:")
    print(f"  Name: {best_model['model_name']}")
    print(f"  SR: {best_model['results']['val_unseen_sampled']['score_summary']['sr']:.3f}")
    print(f"  SPL: {best_model['results']['val_unseen_sampled']['score_summary']['spl']:.3f}")
EOF
