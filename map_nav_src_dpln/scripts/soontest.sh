...
obj_ft_dim=2048
seed=4
MODELS_DIR=${DATA_ROOT}/SOON/exprs_map/pretrain/cmt-vitbase.butdobj-mlm.sap.og-init.lxmert/ckpts
# 输出目录
outdir=${DATA_ROOT}/SOON/exprs_map/batch_test_obj
# 设置总GPU数量
export TOTAL_GPUS=8
...
echo "Dataset: SOON (Object Navigation)"
mkdir -p ${outdir}
...
do
    ...
    accelerate launch --config_file ~/.cache/huggingface/accelerate/gpu${gpu_id}_config.yaml \
        soon/batch_test_models_obj.py $flag \
        --tokenizer bert \
...
if successful_results:
    ...
    best_model = max(successful_results,
                    key=lambda x: x['results']['val_unseen_house']['score_summary']['sr']
                    if 'val_unseen' in x['results'] else 0)

    print(f"\nBest model:")
    print(f"  Name: {best_model['model_name']}")
    print(f"  SR: {best_model['results']['val_unseen_house']['score_summary']['sr']:.3f}")
    print(f"  SPL: {best_model['results']['val_unseen_house']['score_summary']['spl']:.3f}")
EOF
